    ...
    <View className='bg-primary flex-1'>
      ...
      contentContainerStyle={{
        ...
        >
          ...
          <MovieInfo label='Genres' value={movie?.genres?.map((genre) => genre.name).join(' - ' ) || 'N/A'}></MovieInfo>
          <View className='flex flex-row justify-between w-1/2'>
          <MovieInfo label='Budget' value={`$${movie?.budget / 1_000_000} million`}></MovieInfo>
          <MovieInfo label='Revenue' value={`${Math.round(movie?.revenue / 1_000_000)} million`}></MovieInfo>
          </View>
          <MovieInfo label='Production Companies' value={movie?.production_companies.map(company => company.name).join(' , ' )}></MovieInfo>
        </View>
      </ScrollView>
